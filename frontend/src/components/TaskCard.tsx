    ...
    useEffect(() => {
        const handleClick = (e: MouseEvent) => {
            ...
            if (target.tagName === 'IMG') {
                target.classList.toggle("zoom-in", !target.classList.contains("zoom-in"));
            } else {
                const images = document.querySelectorAll('img.zoom-in');
                images.forEach(img => img.classList.remove('zoom-in'));
            }
        };
                    ...
                    <div>
                        ...
                        <div className="space-y-2">
                            {task.acceptanceCriteria.map((criteria, index) => (
                                <div key={index}
                                     className="flex items-start space-x-3 p-3 bg-slate-50 rounded-lg border-l-4 border-indigo-400">
                                    <div
                                        className="w-6 h-6 bg-indigo-100 text-indigo-700 rounded-full flex items-center justify-center text-sm font-medium mt-0.5">
                                        {index + 1}
                                    </div>
